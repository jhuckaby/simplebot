...
sub preg_match {
	...
	my $result = '';
	eval '$result = $string =~ m' . $regexp . '; @$matches = ($&, $1, $2, $3, $4, $5, $6, $7, $8, $9);';
	return $result;
}
...
sub find_timezone_name {
	# try really hard to determine timezone from string
	my $text = shift;
	
	# look for raw offset, such as -0700 or GMT-0700 or GMT-7 or -7.0
	if ($text =~ /^\d{1,2}(\.\d+)?+$/) { $text = "+" . $text; }
	if ($text =~ /(([\+\-])([\d\.]+))/) {
		my ($raw, $prefix, $num) = ($1, $2, $3);
		
		if ($raw !~ /^[\+\-]\d{4}$/) {
			my $hours = int($num);
			my $minutes = int( ($num - int($num)) * 60 );
			$raw = $prefix . sprintf("%02d%02d", $hours, $minutes);
		}
		
		my $tz = undef;
		eval { $tz = DateTime::TimeZone->new( name => $raw ); };
		if ($tz) {
			my $dt = DateTime->now();
			my $offset = $tz->offset_for_datetime($dt);
			
			# okay, now we have a raw offset, find matching timezone for it's name
			foreach my $name (DateTime::TimeZone->all_names()) {
				my $temp_tz = eval { DateTime::TimeZone->new( name => $name ); };
				if ($temp_tz) {
					my $temp_offset = $temp_tz->offset_for_datetime($dt);
	...
	# see if DT::TZ likes it as is, something like America/Los_Angeles
	if (DateTime::TimeZone->is_valid_name($text)) {
		my $dt = DateTime->now();
		my $tz = DateTime::TimeZone->new( name => $text );
		return $tz->name();
	}
...
sub normalize_space {
	# collapse all whitespace into one single character
	my $text = shift;
	$text =~ s/\s+/ /g;
	return trim($text);
}
